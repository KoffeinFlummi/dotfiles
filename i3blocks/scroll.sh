...
if [[ "${OUT_TEXT}" ]]; then
  SYMBOL=$(echo "${OUT_TEXT}" | head -c 3)
  OUT_TEXT=$(echo "${OUT_TEXT}" | tail -c +5)
  ...
  if [[ "${TEXT_LENGTH}" -lt "${MAX_LENGTH}" ]]; then
    ...
    OUT_TEXT=$(printf "%-${MAX_LENGTH}s" "${OUT_TEXT}")
    echo "${SYMBOL} | ${OUT_TEXT}"
    echo "${SYMBOL} | ${OUT_TEXT}"
    sleep "${INTERVAL}"
    exit 0
  fi
  ...
  #echo "<span font_family='monospace'>${SYMBOL}${TEXT_LINE}</span>"
  #echo "<span font_family='monospace'>${SYMBOL}${TEXT_LINE}</span>"
  echo "${SYMBOL} | ${TEXT_LINE}"
  echo "${SYMBOL} | ${TEXT_LINE}"
